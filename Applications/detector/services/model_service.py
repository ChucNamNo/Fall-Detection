...
- Strict Mode: Restores original mathematical formulas for exact benchmark replication.
"""
...
# Cấu hình chống nhiễu cho Robust Mode
...
class PersonState:
    ...
    def extract_kinetics(self, keypoints: np.ndarray, confs: np.ndarray | None = None) -> np.ndarray:
        ...
        Tự động chuyển đổi giữa Strict Mode (Toán học gốc) và Robust Mode (Có fallback).
        """
        if self.strict_mode or confs is None:
            # === STRICT MODE: Công thức chuẩn hóa nguyên bản ===
            ...
            # === ROBUST MODE: Xử lý thông minh khi keypoints bị che khuất ===
...
class CameraState:
    ...
    def match_and_update_persons(
        ...
        """Khớp Tracking ID kết hợp IoU + Fallback Khoảng cách tâm."""
        matched_results: list[tuple[PersonState, np.ndarray, np.ndarray, np.ndarray]] = []
        assigned_person_ids = set()
        unmatched_detections = list(range(len(detected_boxes)))

        # Bước 1: Khớp bằng IoU
        ...
        # Bước 2: Fallback bằng Khoảng cách tâm khi Bounding Box bị biến dạng do ngã
        for det_idx in list(unmatched_detections):
            box = detected_boxes[det_idx]
            min_dist = 180.0
            best_p_id = None
        ...
        # Bước 3: Cấp ID mới cho đối tượng chưa khớp
        ...
        # Bước 4: Dọn dẹp đối tượng mất tích & Trì hoãn reset kinetics
        ...
        for p_id, person in self.persons.items():
            if p_id not in matched_ids:
                ...
                if self.strict_mode:
                    ...
                    # Robust mode: Trì hoãn reset để tránh đứt gãy tín hiệu vận tốc
...
class FallDetectionService:
    ...
    def __init__(
            self,
            strict_mode: bool = False,
            pose_filename: str = "yolo26s-pose.pt"
    ) -> None:
        root = Path(__file__).resolve().parents[2]
        self.model_dir = root / "ai_models"
        self.pose_path = self.model_dir / pose_filename
        self.classifier_path = self.model_dir / "Best BigRU Attention Model.pth"
        self.config_path = self.model_dir / "Best BigRU Attention Config.npy"
    ...
    def _get_state(self, session_id: str) -> CameraState:
        with self._state_lock:
            ...
            if state is None:
                # Quản lý an toàn bộ nhớ: LRU Eviction
                if len(self._states) >= MAX_ACTIVE_SESSIONS:
                    ...
                    logger.warning("Đạt giới hạn MAX_ACTIVE_SESSIONS=%d, loại bỏ session cũ nhất: %s",
                                   MAX_ACTIVE_SESSIONS, oldest_sid)
    ...
    def _extract_all_persons(result: Any) -> tuple[list[np.ndarray], list[np.ndarray], list[np.ndarray]]:
        ...
        for idx in range(len(xy_tensor)):
            ...
            if boxes_tensor is not None and getattr(boxes_tensor, "xyxy", None) is not None and len(
                    boxes_tensor.xyxy) > idx:
    ...
    def predict(self, frame: np.ndarray, session_id: str) -> dict[str, Any]:
        ...
        with self._model_lock:
            yolo_device: int | str = 0 if self.device.type == "cuda" else "cpu"
        ...
        with state.lock:
            ...
            for person, box, keypoints, confs in matched_targets:
                ...
                if len(person.skeleton_buffer) == MAX_LEN:
                    sequence = np.asarray(person.skeleton_buffer, dtype=np.float32)[None, ...]
                    tensor = torch.from_numpy(sequence).to(self.device)
                    with torch.inference_mode():
                        logit = self.classifier(tensor)
                        person.fall_probability = float(torch.sigmoid(logit).item())

                    raw_is_fall = person.fall_probability >= self.threshold
                ...
                person.is_fall = vote_ratio >= VOTE_THRESHOLD
