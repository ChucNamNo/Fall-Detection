...
# Tối ưu hóa số luồng CPU thực thi PyTorch
...
# Cấu hình phát hiện YOLO-Pose tối ưu
YOLO_IMGSZ = 380
YOLO_CONF_THRESHOLD = 0.18         # Ngưỡng nhạy bắt người ở xa/nằm sàn
YOLO_IOU_THRESHOLD = 0.45

# Cấu hình lọc chống nhận diện nhầm đồ vật (Anti-Ghost)
MIN_VALID_KEYPOINTS = 4            # Tối thiểu 4 khớp xương rõ nét
MIN_TORSO_CONF = 0.30              # Ngưỡng khớp thân trên (vai/hông)

# Cấu hình chu kỳ đối tượng & Chống nhiễu
KINETICS_RESET_AFTER_MISSED_FRAMES = 4
MAX_MISSED_FRAMES = 15
LANDMARK_CONF_THRESHOLD = 0.15

# Ngưỡng bộ lọc hình học (Geometric Gate)
MIN_FALL_TORSO_ANGLE = 42.0        # Góc nghiêng thân tối thiểu để kích hoạt ngã (độ)
...
class BiGRUAttentionModel(nn.Module):
    """Mạng BiGRU kết hợp Feed-Forward Soft Attention chuẩn xác theo kiến trúc mô hình."""
...
@dataclass
class PersonState:
    """Động lực học và trạng thái té ngã của từng cá nhân theo thời gian."""
    track_id: int
    ...
    last_confs: np.ndarray | None = None
    ...
    def reset_kinetics(self) -> None:
        """Reset bộ đệm vận tốc và gia tốc khi đối tượng mất dấu quá lâu."""
        self.prev_raw_flat = None
        self.prev_velocity = None
        self.prev_hip_center = None
        self.prev_height = None

    def check_geometric_fall(self, keypoints: np.ndarray | None, box: np.ndarray | None) -> bool:
        """Bộ lọc hình học: Xác nhận tư thế cơ thể có thực sự sụp đổ/nằm ngang hay không."""
        ...
        dy = float(mid_shoulder[1] - mid_hip[1])  # Trục Y hướng xuống trong ảnh
    ...
    def extract_kinetics(self, keypoints: np.ndarray, confs: np.ndarray | None = None) -> np.ndarray:
        """Trích xuất vector đặc trưng 102 chiều (Vị trí 34, Vận tốc 34, Gia tốc 34)."""
        self.last_keypoints = keypoints.copy()
        self.last_confs = confs.copy() if confs is not None else np.ones(17, dtype=np.float32)
...
class CameraState:
    ...
    def match_and_update_persons(
        ...
        # 2. Khớp khoảng cách tâm (Khi Bounding Box biến dạng)
        for det_idx in list(unmatched_detections):
            box = detected_boxes[det_idx]
            min_dist = 180.0
            best_p_id = None
        ...
        # 3. Cấp ID mới cho người mới xuất hiện
        ...
        # 4. Quản lý dọn dẹp các đối tượng không còn xuất hiện
...
class FallDetectionService:
    ...
    def load(self) -> None:
        ...
        with self._load_lock:
            ...
            try:
                ...
                logger.exception("Lỗi nạp mô hình AI:")
                raise
    ...
    def _extract_all_persons(result: Any) -> tuple[list[np.ndarray], list[np.ndarray], list[np.ndarray]]:
        """Trích xuất và lọc bỏ đối tượng đồ vật/người ảo (Anti-Ghost Filtering)."""
        ...
        for idx in range(len(xy_tensor)):
            ...
            # Lọc Anti-Ghost: Yêu cầu ít nhất 4 khớp rõ nét và có khớp thân (vai/hông)
            if conf is not None:
                valid_kps = int(np.sum(conf > 0.25))
                torso_visible = any(conf[i] > MIN_TORSO_CONF for i in [5, 6, 11, 12])
                if valid_kps < MIN_VALID_KEYPOINTS or not torso_visible:
                    continue
    ...
    def predict(self, frame: np.ndarray, session_id: str) -> dict[str, Any]:
        ...
        with state.lock:
            ...
            # Trích xuất đặc trưng cho các đối tượng phát hiện trực tiếp
            ...
            for person, box, keypoints, confs in matched_targets:
                ...
                persons_output.append({
                    ...
                    "bbox": [round(float(v), 1) for v in box] if box is not None else [],
                    "keypoints": points,
                })
