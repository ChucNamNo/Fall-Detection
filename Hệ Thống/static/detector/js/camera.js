(() => {
  ...
  function drawPose(data) {
    ...
    // 1. Trích xuất danh sách persons từ backend
    ...
    persons.forEach(person => {
      ...
      const boxColor = isFall ? 'rgba(255, 82, 103, 0.85)' : 'rgba(52, 214, 209, 0.85)';

      // --- A. VẼ BOUNDING BOX & NHÃN TRACK ID ---
      if (person.bbox && person.bbox.length === 4) {
        ...
        poseCtx.lineWidth = 2;
        poseCtx.strokeStyle = boxColor;
        poseCtx.strokeRect(bx, by, bw, bh);

        // Vẽ nhãn Track ID và trạng thái trên đầu Bbox
        const label = `ID: ${person.track_id} | ${person.status} (${(person.fall_probability * 100).toFixed(0)}%)`;
        poseCtx.font = 'bold 12px Plus Jakarta Sans, sans-serif';
        const textWidth = poseCtx.measureText(label).width;
        
        poseCtx.fillStyle = boxColor;
        poseCtx.fillRect(bx, Math.max(0, by - 20), textWidth + 10, 20);
        
        poseCtx.fillStyle = '#0d1117';
        poseCtx.fillText(label, bx + 5, Math.max(14, by - 5));
      }

      // --- B. VẼ 17 KEYPOINTS & XƯƠNG LIÊN KẾT ---
      const rawKps = person.keypoints || [];
      // Chuẩn hóa sang mảng [x, y, conf] để tương thích
      const keypoints = rawKps.map(pt => Array.isArray(pt) ? pt : [pt.x, pt.y, pt.conf ?? 1.0]);

      // Vẽ các đường nối chi
  ...
  function updateStatus(data) {
    ...
    const hasPerson = (data.person_count > 0);
